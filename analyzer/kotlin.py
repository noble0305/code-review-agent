"""Kotlin analyzer using regex-based analysis."""
import re
...
class KotlinAnalyzer(BaseAnalyzer):
    ...
    def _complexity(self, fl):
        ...
        for fp, lines in fl.items():
            ...
            for i, l in enumerate(lines, 1):
                ...
                if re.match(r'^\s*(if|else|when|for|while|do|try|catch)\b', s): depth += 1
                if depth > 5:
    ...
    def _naming(self, fl):
        ...
        for fp, lines in fl.items():
            for i, l in enumerate(lines, 1):
                s = l.strip()
                m = re.match(r'fun\s+([A-Z]\w+)', s)
                if m:
                    dim.score -= 2; dim.issues.append(Issue(severity='info', file_path=fp, line_number=i,
                        description=f'函数名 "{m.group(1)}" 应使用 camelCase', suggestion='Kotlin 函数名使用 camelCase', metric=m.group(1)))
                m = re.match(r'class\s+([a-z]\w*)', s)
                if m:
                    dim.score -= 2; dim.issues.append(Issue(severity='info', file_path=fp, line_number=i,
                        description=f'类名 "{m.group(1)}" 应使用 PascalCase', suggestion='Kotlin 类名使用 PascalCase', metric=m.group(1)))
    ...
    def _func_length(self, fl):
        ...
        for fp, lines in fl.items():
            ...
            for i, l in enumerate(lines, 1):
                s = l.strip()
                if fs is None:
                    if re.match(r'\s*fun\s+\w+', s) and '{' in s:
            ...
            for i, l in enumerate(lines, 1):
                ...
                if re.search(r'!!\s*$', s):
                    dim.issues.append(Issue(severity='warning', file_path=fp, line_number=i,
                        description='使用了 !! 非空断言可能 NPE', suggestion='使用 ?. 或 let 安全处理', metric=s[:60]))
    ...
    def _idioms(self, fl):
        dim = DimensionScore(name="Kotlin 惯用法", score=100, weight=0.11)
        for fp, lines in fl.items():
            for i, l in enumerate(lines, 1):
                s = l.strip()
                if re.search(r':\s*java\.util\.\w+', s):
                    dim.score -= 2; dim.issues.append(Issue(severity='info', file_path=fp, line_number=i,
                        description='使用了 Java 集合类型', suggestion='使用 Kotlin 集合 (List, Map)', metric=s[:60]))
        dim.score = max(0, dim.score); return dim
        ...
        for fp, lines in fl.items():
            for i, l in enumerate(lines, 1):
                ...
                if re.match(r'class\s+\w+', s):
                    methods = sum(1 for ll in lines if re.match(r'\s*fun\s+', ll.strip()))
